...
function renderTablaFletes() {
    ...
    if (paginatedItems.length === 0) {
        tbody.innerHTML = '<tr><td colspan="7" style="text-align:center; padding:2rem; color:var(--text-muted);">No se encontraron fletes</td></tr>';
        return;
    }
    ...
    paginatedItems.forEach(f => {
        ...
        if (f.estado === 'sin_asignar') {
            ...
            btn.onclick = () => location.href = `optimizador-de-rutas-ia.html?flete=${f.cod_flete}`;
        } else {
...
function verDetalleFlete(codFlete) {
    ...
    document.getElementById('panel-flete-titulo').innerText = `Flete: ${f.cod_flete}`;
    ...
    const htmlInfo = `
        <div class="vehiculo-detail-grid">
            ...
            <div class="detail-item"><label>Punto de Carga</label><span>${f.punto_carga}</span></div>
            <div class="detail-item"><label>Destino</label><span>${f.destino_nombre || 'No definido'}</span></div>
            ...
            <div class="detail-item" style="grid-column: span 1;">
                ...
                <div style="font-size: 0.8rem; font-weight: 800; margin-top: 5px;">${f.origen}</div>
            </div>
            <div class="detail-item" style="grid-column: span 1;">
                <label>Coordenadas Destino</label>
                <div style="font-size: 0.8rem; font-weight: 800; margin-top: 5px;">${f.destino}</div>
            </div>
        </div>
    `;
...
async function desasignarCamion(idFlete) {
    ...
    try {
        ...
        Notificacion.fire({ icon: 'error', title: 'Error al liberar: ' + e.message });
    }
}
